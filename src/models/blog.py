...
class Blog(object):
    ...
    @staticmethod
    def del_blog(id):
        del_blog = Database.delete_one(collection='blogs', query={'_id': id})
        return str(del_blog.deleted_count)

    def update_blog(self, id):
        Database.update(collection='blogs', query={'_id': id}, data=self.json())
